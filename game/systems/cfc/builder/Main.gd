...
func _ready():
	...
	_menubar.connect("create_new_card_project", self, "_on_create_new_project")
	_menubar.connect("card_project_selected", self, "_on_project_selected")
	_menubar.connect("card_project_closed", self, "_on_project_closed")

	_main_menu.connect("open_card_project", self, "_on_open_project")
	_main_menu.connect("save_card_project", self, "_on_save_project")
...
func _exit_tree():
	...
	CardProjectManager.remove_all_projects()
...
func _create_active_default_project() -> void:
	var default_project: CardProject = CardProjectManager.add_project()
...
func _on_project_selected(project_id: int) -> void:
	var project: CardProject = CardProjectManager.get_project_by_id(project_id)
...
func _close_project(project_id: int) -> void:
	...
	# Remove project
	CardProjectManager.remove_project(project)
	_menubar.remove_tab(project)

	# Choose new project if active tab was closed
	if active_project_closed:
		if CardProjectManager.get_project_count() == 0:
			_create_active_default_project()
		else:
			var new_project_id: int = _menubar.get_first_project_id()
			var new_project: CardProject = CardProjectManager.get_project_by_id(new_project_id)
...
func _on_open_project(filepath: String) -> bool:
	...
	var project: CardProject = CardProjectManager.get_open_project_by_filepath(filepath)
	var active_project: CardProject = CardProjectManager.get_active_project()
	...
	if active_project.filepath.empty() && !active_project.dirty:
		CardProjectManager.remove_project(active_project)
		_menubar.remove_tab(active_project)

	# Create and open it
	project = CardProjectManager.add_project(filepath)
	_make_project_active(project)
...
func _on_exit_with_changes_saved(project_ids: Array) -> void:
	if CardProjectManager.has_unsaved_projects():
		pass
	else:
		CardProjectManager.save_all_projects()
		get_tree().quit()
