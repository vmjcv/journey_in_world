...
const CARD_UPGRADE_SCENE = preload("res://addons/cfc/src/dreamscape/ChoiceCardObject.tscn")
...
func _on_card_upgrade_selected(option: int, draft_card_object) -> void:
	...
	if select_return is GDScriptFunctionState: # Still working.
		...
		if typeof(select_return) == TYPE_ARRAY:
			...
			draft_card_object.display_card.card_front.apply_sharer("res://addons/cfc/shaders/grayscale.shader")
